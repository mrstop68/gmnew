...
//images apisindeki aktif sayfa resimlerini tespit etme
    ...
    class FilterPagesToLangCode{
        ...
        function __construct($parameterValue,$numberAD=null)
        {
        ...
        function langFindBlog($data){
            if(empty($this->parameterValue))$this->parameterValue='mainlang';
            return strtolower($data->lang)==$this->parameterValue;
        }
        function popupSelect($popupdata){
            ...
            if($this->parameterValue > $fromDate && $this->parameterValue < $toDate &&  $popupdata->title == $this->numberAD){
                return true;
            }else{
                return false;
            }
    ...
    class mergeContent{
       public static function selectedLangContent($trContent,$otherLangContent){
            $mergedArray = [];

            foreach ($trContent as $item) {
                $editorid = $item->editorid;
                if (!isset($mergedArray[$editorid])) {
                    $mergedArray[$editorid] = $item;
                } else {
                    $mergedArray[$editorid]->content = array_merge($mergedArray[$editorid]->content, $item->content);
                }
            }

            foreach ($otherLangContent as $item) {
                $editorid = $item->editorid;
                if (!isset($mergedArray[$editorid])) {
                    $mergedArray[$editorid] = $item;
                } else {
